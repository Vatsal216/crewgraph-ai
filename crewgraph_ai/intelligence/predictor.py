...
from ..types import TaskResult, WorkflowId
from ..utils.logging import get_logger
...
class PerformancePredictor:
    ...
    def extract_workflow_features(self, workflow_data: Dict[str, Any]) -> WorkflowFeatures:
        ...
        task_complexity = sum(task.get("complexity", 1.0) for task in tasks) / max(task_count, 1)

        # Estimate data size from inputs
        data_size = sum(len(str(task.get("input", ""))) for task in tasks)
        ...
        memory_requirements = sum(task.get("memory_mb", 128) for task in tasks)

        cpu_requirements = sum(task.get("cpu_cores", 0.5) for task in tasks)
    ...
    def _calculate_success_probability(
        self, features: WorkflowFeatures, execution_time: float, bottleneck_probability: float
    ) -> float:
        ...
        success_probability = (
            base_probability - complexity_penalty - time_penalty - bottleneck_penalty
        )
    ...
    def _estimate_cost(self, features: WorkflowFeatures, execution_time: float) -> float:
        ...
        compute_cost = (
            features.cpu_requirements * execution_time * 0.0001
        )  # $0.0001 per core-second
        memory_cost = (
            features.memory_requirements * execution_time * 0.00001
        )  # $0.00001 per MB-second
    ...
    def _record_prediction(
        self, features: WorkflowFeatures, prediction: PerformancePrediction, prediction_time: float
    ):
...
class ResourcePredictor:
    ...
    def _record_resource_prediction(
        self, workflow_data: Dict[str, Any], prediction: ResourcePrediction, prediction_time: float
    ):
    ...
    def predict_execution_time(
        self, workflow_id: WorkflowId, task_count: int, context: Optional[Dict[str, Any]] = None
    ) -> Tuple[float, float]:
        """
        ...
        logger.debug(
            f"Predicted execution time: {predicted_time:.2f}s (confidence: {confidence:.2f})"
        )
        return predicted_time, confidence

    def predict_resource_usage(
        self, workflow_id: WorkflowId, task_count: int
    ...
    def detect_performance_anomalies(self, metrics: WorkflowMetrics) -> List[str]:
        """
        ...
        if recent_metrics:
            ...
            if metrics.execution_time > avg_execution_time * 2:
                anomalies.append(
                    f"Execution time {metrics.execution_time:.2f}s is 2x higher than average {avg_execution_time:.2f}s"
                )

            if metrics.memory_usage > avg_memory * 2:
                anomalies.append(
                    f"Memory usage {metrics.memory_usage:.1f}MB is 2x higher than average {avg_memory:.1f}MB"
                )

            if metrics.cpu_usage > avg_cpu * 1.5:
                anomalies.append(
                    f"CPU usage {metrics.cpu_usage:.1f}% is 1.5x higher than average {avg_cpu:.1f}%"
                )

            if metrics.success_rate < 0.8:
                anomalies.append(
                    f"Success rate {metrics.success_rate:.2f} is below acceptable threshold (0.8)"
                )

        if anomalies:
            logger.warning(
                f"Detected {len(anomalies)} performance anomalies for workflow {metrics.workflow_id}"
            )

        return anomalies

    def get_optimization_recommendations(self, workflow_id: WorkflowId) -> List[str]:
        """
        ...
        if recent_metrics:
            ...
            if avg_success_rate < 0.9:
                recommendations.append(
                    "Consider implementing retry mechanisms for improved reliability"
                )

            if avg_execution_time > 30.0:
                recommendations.append(
                    "Workflow execution time is high - consider parallel task execution"
                )
            ...
            if len(high_memory_executions) > len(recent_metrics) * 0.3:
                recommendations.append(
                    "Frequent high memory usage detected - implement memory optimization"
                )
            ...
            if len(high_cpu_executions) > len(recent_metrics) * 0.2:
                recommendations.append(
                    "CPU usage often high - consider task distribution or resource scaling"
                )

        return recommendations

    def _find_similar_workflows(
        self, task_count: int, workflow_id: WorkflowId, tolerance: int = 2
        ...
        if len(similar) < 3:
            similar = [
                m for m in self.metrics_history if abs(m.task_count - task_count) <= tolerance * 2
            ]
    ...
    def _estimate_prediction_accuracy(self) -> float:
        ...
        if len(recent_times) > 5:
            coefficient_of_variation = statistics.stdev(recent_times) / statistics.mean(
                recent_times
            )
