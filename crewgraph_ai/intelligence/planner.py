...
class AdaptivePlanner:
    ...
    def adapt_during_execution(
        ...
        remaining_tasks: int,
    ) -> Optional[PlanningRecommendation]:
        """
        ...
        if should_adapt:
            logger.info(
                f"Adapting strategy from {current_strategy.value} to {new_strategy.value}: {adaptation_reason}"
            )
    ...
    def analyze_execution_patterns(self, execution_history: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for strategy, executions in strategy_groups.items():
            if executions:
                ...
                avg_success_rate = sum(e.get("success_rate", 0) for e in executions) / len(
                    executions
                )
        ...
        if recent_executions:
            recent_avg_time = sum(e.get("execution_time", 0) for e in recent_executions) / len(
                recent_executions
            )
            all_avg_time = sum(e.get("execution_time", 0) for e in execution_history) / len(
                execution_history
            )
    ...
    def _should_use_adaptive_strategy(self, workflow_definition: Dict[str, Any]) -> bool:
        ...
        has_complex_dependencies = (
            len(workflow_definition.get("dependencies", [])) > task_count // 2
        )
        ...
        return (
            task_count >= 5
            or has_complex_dependencies
            or workflow_definition.get("resource_requirements") is None
        )

    def _estimate_resource_requirements(
        self, task_count: int, strategy: PlanningStrategy, workflow_definition: Dict[str, Any]
    ) -> Dict[str, float]:
