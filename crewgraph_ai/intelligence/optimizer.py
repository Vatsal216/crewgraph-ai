...
from ..types import TaskStatus, WorkflowId
from ..utils.logging import get_logger
...
class WorkflowOptimizer:
        ...
        with self._lock:
            ...
            for iteration in range(max_iterations):
                ...
                if recommendations:
                    ...
                    # Evaluate candidate
                    candidate_performance = self.performance_predictor.predict_performance(
                        candidate_workflow
                    )
                    candidate_resources = self.resource_predictor.predict_resources(
                        candidate_workflow
                    )
                    ...
                    if candidate_score > best_score:
                        ...
                        logger.info(
                            f"Applied optimization: {best_recommendation.optimization_type}"
                        )
                    else:
            ...
            improvements = self._calculate_improvements(
                baseline_performance, baseline_resources, final_performance, final_resources
            )
    ...
    def _generate_optimization_recommendations(
        self, workflow_data: Dict[str, Any], strategy: OptimizationStrategy
    ...
    def _recommend_task_optimizations(
        self, tasks: List[Dict[str, Any]], strategy: OptimizationStrategy
    ) -> List[OptimizationRecommendation]:
        ...
        if complex_tasks and strategy in [
        ...
        if resource_heavy_tasks and strategy in [
    ...
    def _recommend_agent_optimizations(
        self, agents: List[Dict[str, Any]], strategy: OptimizationStrategy
    ) -> List[OptimizationRecommendation]:
        """Recommend agent-level optimizations"""
        recommendations = []

        # Agent pooling optimization
        if len(agents) > 5 and strategy in [
            OptimizationStrategy.RESOURCE,
            OptimizationStrategy.COST,
        ]:
            recommendations.append(
        ...
        if len(agents) > 3 and strategy in [
            OptimizationStrategy.PERFORMANCE,
            OptimizationStrategy.BALANCED,
        ]:
            recommendations.append(
    ...
    def _recommend_structure_optimizations(
        self,
        ...
        if sequential_chains and strategy in [
            OptimizationStrategy.PERFORMANCE,
            OptimizationStrategy.BALANCED,
        ]:
            recommendations.append(
    ...
    def _recommend_resource_optimizations(
        self, workflow_data: Dict[str, Any], strategy: OptimizationStrategy
    ) -> List[OptimizationRecommendation]:
    ...
    def _apply_optimization(
        self, workflow_data: Dict[str, Any], recommendation: OptimizationRecommendation
    ) -> Dict[str, Any]:
    ...
    def _calculate_optimization_score(
        self, performance_prediction, resource_prediction, strategy: OptimizationStrategy
    ) -> float:
    ...
    def _calculate_improvements(
        self, baseline_performance, baseline_resources, final_performance, final_resources
    ) -> Dict[str, float]:
        ...
        improvements["resource_usage_improvement"] = (
            improvements["memory_usage_improvement"] + improvements["cpu_usage_improvement"]
        ) / 2.0
    ...
    def get_optimization_statistics(self) -> Dict[str, Any]:
        ...
        performance_gains = [o["result"]["performance_gain"] for o in self._optimization_history]
        ...
        return {
            ...
            "created_by": "Vatsal216",
            "timestamp": time.time(),
            """Result of resource optimization analysis.""" "optimization_type": OptimizationType,
...
class ResourceOptimizer:
    ...
    def __init__(self, max_cpu_threshold: float = 80.0, max_memory_threshold: float = 85.0):
        ...
        logger.info(
            f"ResourceOptimizer initialized with CPU threshold: {max_cpu_threshold}%, "
    ...
    def optimize_memory_usage(self, current_workflows: List[WorkflowId]) -> OptimizationResult:
        """
        ...
        logger.info(
            f"Memory optimization analysis complete: {potential_improvement:.1f}% potential improvement"
        )

        return result

    def optimize_cpu_usage(
        self, task_count: int, parallel_execution: bool = True
    ) -> OptimizationResult:
        """
        ...
        self.optimization_history.append(result)
        logger.info(
            f"CPU optimization analysis complete: {potential_improvement:.1f}% potential improvement"
        )

        return result

    def optimize_task_scheduling(self, tasks: List[Dict[str, Any]]) -> OptimizationResult:
        """
        ...
        if len(cpu_intensive_tasks) > psutil.cpu_count():
            recommendations.append(
                f"Serialize CPU-intensive tasks to prevent overload "
        ...
        self.optimization_history.append(result)
        logger.info(
            f"Task scheduling optimization complete: {potential_improvement:.1f}% potential improvement"
        )

        return result

    def get_resource_allocation_strategy(
        self, workflow_requirements: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        if current_resources.get("cpu_percent", 0) > 70:
            ...
            strategy["optimization_recommendations"].append(
                "System under high load - consider delayed execution"
            )

        if current_resources.get("memory_percent", 0) > 80:
            strategy["memory_allocation_gb"] = min(required_memory, available_memory * 0.5)
            strategy["optimization_recommendations"].append(
                "Memory pressure detected - reducing allocation"
            )

        return strategy

    async def monitor_resource_trends(self, duration_minutes: int = 10) -> Dict[str, List[float]]:
        """
        ...
        """
        trends = {"cpu_usage": [], "memory_usage": [], "timestamps": []}
        ...
        if trends["cpu_usage"]:
            ...
            logger.info(
                f"Resource monitoring complete - CPU avg: {cpu_avg:.1f}%, "
    ...
    def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "optimization_types": list(
                set(opt.optimization_type.value for opt in self.optimization_history)
            ),
