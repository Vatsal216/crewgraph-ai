...
class MLModelManager:
    ...
    def predict(self, 
               model_type: ModelType, 
               features: np.ndarray) -> Union[np.ndarray, Tuple[np.ndarray, np.ndarray]]:
        """
    ...
    def generate_synthetic_training_data(self, 
                                       model_type: ModelType,
                                       num_samples: int = 1000) -> TrainingData:
        """
        ...
        if model_type == ModelType.PERFORMANCE_PREDICTOR:
            ...
            bottleneck_score = (features[:, 0] * 0.02 + 
                               features[:, 1] * 0.001 + 
                               features[:, 2] * 0.05 - 
                               features[:, 3] * 0.001)
