...
from .base import EdgeType, ExecutionPlan, NodeType, PlanEdge, PlanningStrategy, PlanNode
...
class SequentialStrategy(PlanningStrategy):
    ...
    def create_plan(
        ...
        for i, task in enumerate(tasks):
            ...
            if prev_node_id:
                edge = PlanEdge(
                    from_node=prev_node_id, to_node=node.id, edge_type=EdgeType.SEQUENTIAL
                )
    ...
    def create_plan(
            ...
            for task in group:
                ...
                if prev_group_nodes:
                    for prev_node in prev_group_nodes:
                        edge = PlanEdge(
                            from_node=prev_node.id, to_node=node.id, edge_type=EdgeType.PARALLEL
                        )
                        plan.edges.append(edge)
        ...
        for task in dependent_tasks:
            ...
            if prev_group_nodes:
                for prev_node in prev_group_nodes:
                    edge = PlanEdge(
                        from_node=prev_node.id, to_node=node.id, edge_type=EdgeType.SEQUENTIAL
                    )
                    plan.edges.append(edge)
...
class ConditionalStrategy(PlanningStrategy):
    ...
    def create_plan(
        ...
        logger.info(
            f"Conditional plan created with {len(reliable_tasks)} reliable and {len(unreliable_tasks)} unreliable tasks"
        )
        return plan
...
class OptimalStrategy(PlanningStrategy):
    ...
    def create_plan(
            ...
            if self._can_run_parallel(task, dependency_graph, parallel_candidates):
                ...
                if len(parallel_candidates) > 1:
                    for candidate_name in parallel_candidates[:-1]:
                        candidate_node = next(
                            n for n in plan.nodes if n.task_name == candidate_name
                        )
                        edge = PlanEdge(
                ...
                max_parallel_duration = max(
                    task_scores[name]["estimated_duration"] for name in parallel_candidates
                )
        ...
        logger.info(
            f"Optimal plan created with optimization score: {plan.metadata['optimization_score']:.2f}"
        )
        return plan

    def _calculate_task_scores(
        self, tasks: List[Any], task_analysis: Dict[str, Any]
    ...
    def _can_run_parallel(
        self, task: Any, dependency_graph: Dict[str, List[str]], current_parallel: List[str]
    ) -> bool:
    ...
    def _calculate_optimization_score(
        ...
        optimization_score = (
            parallelization_factor * 0.4 + dependency_efficiency * 0.4 + resource_efficiency * 0.2
        )
    ...
    def _calculate_resource_efficiency(
        self, plan: ExecutionPlan, resource_analysis: Dict[str, Any]
    ) -> float:
        """Calculate resource utilization efficiency"""

        total_estimated_cpu = sum(node.estimated_resources.get("cpu", 1.0) for node in plan.nodes)

        total_estimated_memory = sum(
            node.estimated_resources.get("memory", 100.0) for node in plan.nodes
        )
        ...
        memory_efficiency = (
            min(1.0, total_estimated_memory / available_memory) if available_memory > 0 else 1.0
        )
