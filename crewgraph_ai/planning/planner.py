...
from .base import BasePlanner, ExecutionPlan, PlanEdge, PlanningStrategy, PlanNode
...
class DynamicPlanner(BasePlanner):
    ...
    def __init__(self, config: Optional[PlannerConfig] = None):
        ...
        self._current_strategy = self._strategies.get(
            self.config.strategy, self._strategies["optimal"]
        )
    ...
    async def create_plan_async(
        ...
        return await loop.run_in_executor(
            self._executor, self.create_plan, tasks, state, constraints
        )

    def replan(
        self, plan_id: str, current_state: SharedState, execution_feedback: Dict[str, Any]
        ...
        with self._lock:
            ...
            try:
                ...
                new_plan = self.create_plan(
                    tasks=remaining_tasks, state=current_state, constraints=current_plan.constraints
                )

                # Update plan relationships
                new_plan.parent_plan_id = plan_id
                new_plan.replan_reason = execution_feedback.get(
    ...
    def update_performance_model(self, task_name: str, execution_result: Dict[str, Any]) -> None:
        """
        ...
        with self._lock:
            ...
            if "avg_execution_time" in model:
                ...
                model["avg_execution_time"] = (
                    model["avg_execution_time"] * count + execution_time
                ) / (count + 1)
    ...
    def get_optimal_task_order(
        self, tasks: List[TaskWrapper], constraints: Optional[Dict[str, Any]] = None
    ) -> List[str]:
        """
        ...
        with self._lock:
            try:
                ...
                ordered_tasks = self._topological_sort_with_priorities(
                    dependency_graph, task_priorities
                )
    ...
    def analyze_workflow_bottlenecks(self, plan: ExecutionPlan) -> List[Dict[str, Any]]:
        """
    ...
    def get_planner_stats(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            successful_plans = sum(1 for h in self._execution_history if h.get("success", False))

            success_rate = successful_plans / max(total_plans, 1)

            # Calculate average planning time
            avg_planning_time = sum(
                h.get("planning_time", 0.0) for h in self._execution_history
            ) / max(total_plans, 1)
    ...
    def _analyze_tasks(self, tasks: List[TaskWrapper], state: SharedState) -> Dict[str, Any]:
        ...
        for task in tasks:
            ...
            estimated_time = self.predict_execution_time(task.name, {"state": state.to_dict()})
    ...
    def _analyze_resources(
        self, tasks: List[TaskWrapper], constraints: Optional[Dict[str, Any]]
    ) -> Dict[str, Any]:
    ...
    def _optimize_plan(self, plan: ExecutionPlan, task_analysis: Dict[str, Any]) -> ExecutionPlan:
    ...
    def _optimize_parallelization(
        self, plan: ExecutionPlan, task_analysis: Dict[str, Any]
    ) -> ExecutionPlan:
    ...
    def _add_performance_predictions(self, plan: ExecutionPlan, tasks: List[TaskWrapper]) -> None:
    ...
    def _should_replan(
        self, current_plan: ExecutionPlan, execution_feedback: Dict[str, Any]
    ) -> bool:
    ...
    def _calculate_task_priorities(
        ...
        for task in tasks:
            ...
            dependents = sum(1 for t in tasks if task.name in t.dependencies)
    ...
    def _topological_sort_with_priorities(
        self, dependency_graph: Dict[str, List[str]], priorities: Dict[str, int]
    ) -> List[str]:
    ...
    def __repr__(self) -> str:
        return (
            f"DynamicPlanner(strategy='{self.config.strategy}', "
