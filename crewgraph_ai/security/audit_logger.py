...
class AuditLogger:
    ...
    def _flush_buffer(self):
        ...
        try:
            ...
            if (
                self.config.enable_file_rotation
                and self._current_file_size > self.config.max_file_size_mb * 1024 * 1024
            ):
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        with self._lock:
            stats = {
                ...
                "events_by_category": {
                    cat.value: count for cat, count in self._events_by_category.items()
                },
                "events_by_level": {
                    level.value: count for level, count in self._events_by_level.items()
                },
                "current_file_size_mb": (
                    round(self._current_file_size / (1024 * 1024), 2) if self._log_file else 0
                ),
