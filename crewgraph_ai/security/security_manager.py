...
class SecuritySession:
    ...
    def is_valid(self) -> bool:
        ...
        return self.status == SessionStatus.ACTIVE and now < self.expires_at
...
class SecurityManager:
    ...
    def create_user(
        ...
        """
        user = User(
            user_id=user_id, username=username, email=email, roles=roles, metadata=metadata or {}
        )
    ...
    def authenticate_user(
        ...
        if (
            self._default_policy.ip_whitelist
            and ip_address
            and ip_address not in self._default_policy.ip_whitelist
        ):
        ...
        session = SecuritySession(
            ...
            + timedelta(seconds=self.default_session_duration),
    ...
    def decrypt_data(
        self, encrypted_data: str, context: Optional[SecurityContext] = None
    ) -> Optional[Any]:
