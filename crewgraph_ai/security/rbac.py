    ...
    def validate_rbac_integrity(self) -> Dict[str, Any]:
        ...
        for role_name in self._roles:
            ...
            if self._has_circular_dependency(role_name, visited):
                issues.append(
                    f"Circular dependency detected in role hierarchy starting from '{role_name}'"
                )

        return {"is_valid": len(issues) == 0, "issues": issues, "issues_count": len(issues)}

    def _has_circular_dependency(
        self, role_name: str, visited: Set[str], path: Optional[Set[str]] = None
    ) -> bool:
