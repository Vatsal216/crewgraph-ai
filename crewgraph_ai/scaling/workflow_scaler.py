...
class WorkflowAutoScaler:
    ...
    def start(self) -> None:
        ...
        self._monitor_thread = threading.Thread(
            ...
            name="WorkflowAutoScaler-Monitor",
            daemon=True
        )
        ...
        if self.enable_predictive:
            self._predictor_thread = threading.Thread(
                target=self._predictive_loop,
                name="WorkflowAutoScaler-Predictor", 
                daemon=True
            )
        ...
        metrics.increment_counter(
            "crewgraph_autoscaler_started_total",
    ...
    def _evaluate_scaling_rules(self, metrics: Dict[str, float]) -> ScalingDirection:
        ...
        for rule in self.scaling_rules:
            ...
            if (metric_value >= rule.threshold_up and 
                ...
                scale_up_votes += 1
                logger.debug(f"Rule '{rule.name}': Scale UP vote (value={metric_value}, threshold={rule.threshold_up})")
            
            elif (metric_value <= rule.threshold_down and 
                  current_instances > rule.min_instances):
                scale_down_votes += 1
                logger.debug(f"Rule '{rule.name}': Scale DOWN vote (value={metric_value}, threshold={rule.threshold_down})")
    ...
    def _execute_scaling(self, direction: ScalingDirection, metrics: Dict[str, float], predictive: bool = False) -> None:
    ...
    def _scale_up(self, metrics: Dict[str, float], predictive: bool = False) -> None:
        ...
        if success_count > 0:
            ...
            logger.info(f"Scaled UP by {success_count} instances "
    ...
    def _scale_down(self, metrics: Dict[str, float], predictive: bool = False) -> None:
        ...
        if success_count > 0:
            ...
            logger.info(f"Scaled DOWN by {success_count} instances "
                       f"({'predictive' if predictive else 'reactive'}). "
                       f"Total instances: {len(self.instances)}")
...
class WorkflowLoadBalancer:
    ...
    def select_instance(self, instances: Dict[str, WorkflowInstance], 
                       request_info: Optional[Dict] = None) -> Optional[str]:
        ...
        if self.strategy == "round_robin":
            ...
            return min(available_instances, 
                      key=lambda id: instances[id].task_count)
