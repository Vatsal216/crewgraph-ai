...
class RequirementsParser:
    ...
    def _extract_tasks(self, text: str) -> List[ParsedTask]:
        ...
        for sentence in sentences:
            ...
            if task_info:
                task_name = (
                    f"task_{task_counter}" if not task_info.get("name") else task_info["name"]
                )
...
class WorkflowParser:
    ...
    def parse_description(
        ...
        for task_dict in parsed_data.get("tasks", []):
            if isinstance(task_dict, dict):
                ...
                task = {
                    ...
                    "type": (
                        task_dict.get("task_type", {}).get("value", "general")
                        if isinstance(task_dict.get("task_type"), dict)
                        else str(task_dict.get("task_type", "general"))
                    ),
                    "estimated_duration": int(
                        task_dict.get("estimated_complexity", 1.0) * 10
                    ),  # Convert complexity to minutes
        ...
        logger.info(
            f"Parsed workflow '{workflow_name}' with {len(tasks)} tasks "
    ...
    def parse_conversational_input(
        self, conversation_history: List[str], current_input: str
    ) -> ParsedWorkflow:
        """
        ...
        logger.info(
            f"Parsed conversational workflow with {len(conversation_history)} context turns"
        )
        return parsed
    ...
    def enhance_workflow_with_suggestions(self, parsed_workflow: ParsedWorkflow) -> ParsedWorkflow:
        """
    ...
    def _extract_workflow_name(self, description: str) -> str:
        ...
        if "data" in description and "process" in description:
            ...
            meaningful_words = [
                w for w in words if len(w) > 3 and w not in ["the", "and", "for", "with"]
            ]
            if meaningful_words:
    ...
    def _extract_metadata(
        ...
        if any(keyword in description for keyword in ["urgent", "asap", "immediately", "critical"]):
            ...
            keyword in description for keyword in ["low priority", "when possible", "eventually"]
        ):
    ...
    def _apply_refinements(
        self, parsed: ParsedWorkflow, refinements: Dict[str, Any]
    ) -> ParsedWorkflow:
    ...
    def _calculate_completeness(self, parsed_workflow: ParsedWorkflow) -> float:
        ...
        complete_tasks = [
            t
            for t in parsed_workflow.tasks
    ...
    def _suggest_missing_tasks(self, parsed_workflow: ParsedWorkflow) -> List[Dict[str, Any]]:
        ...
        if parsed_workflow.workflow_type == WorkflowType.DATA_PROCESSING:
            ...
            has_validation = any(
                "valid" in task.get("description", "").lower() for task in parsed_workflow.tasks
            )
            ...
            has_output = any(
                "save" in task.get("description", "").lower()
                or "output" in task.get("description", "").lower()
                for task in parsed_workflow.tasks
            )
            if not has_output:
                suggested.append(
    ...
    def _generate_suggested_dependencies(self, tasks: List[Dict[str, Any]]) -> List[Dict[str, str]]:
        ...
        for suggested in suggested_tasks:
            if suggested.get("type") == "validation" and original_tasks:
                ...
                dependencies.append({"source": suggested["id"], "target": original_tasks[0]["id"]})
            elif suggested.get("type") in ["io", "logging"] and original_tasks:
                # Output tasks should come last
                dependencies.append({"source": original_tasks[-1]["id"], "target": suggested["id"]})
