...
from .parser import ParsedAgent, ParsedTask, ParsedWorkflow, RequirementsParser, WorkflowParser
...
class NLToWorkflowConverter:
    ...
    def convert_to_workflow(
        self, natural_language: str, workflow_format: str = "crewai"
    ) -> Dict[str, Any]:
        """
        ...
        with self._lock:
            ...
            self._record_conversion(
                natural_language, parsed_workflow, workflow_def, workflow_format
            )
    ...
    def _convert_to_crewai(self, parsed_workflow: ParsedWorkflow) -> Dict[str, Any]:
        ...
        for task in parsed_workflow.tasks:
            task_def = {
                ...
                "context": [],
                "config": {"max_execution_time": None, "max_retries": 3, "retry_delay": 1},
                "output_json": None,
    ...
    def _convert_to_langgraph(self, parsed_workflow: ParsedWorkflow) -> Dict[str, Any]:
        ...
        for task in parsed_workflow.tasks:
            node_def = {
                ...
                "metadata": {
                    ...
                    "complexity": task.estimated_complexity,
                    "agent_role": (
                        task.agent_role_required.value if task.agent_role_required else None
                    ),
                    "created_by": "Vatsal216",
                },
            }
        ...
        for from_task, to_task in parsed_workflow.dependencies:
            edge_def = {
                ...
                "metadata": {"dependency_type": "sequential", "created_by": "Vatsal216"},
            }
        ...
        langgraph_config = {
            ...
            "checkpointer": {"type": "memory", "config": {}},
            "interrupt_before": [],
            "interrupt_after": [],
            "debug": False,
            "stream_mode": "values",
        ...
        return {
            ...
            "execution_config": {
                ...
                "config": {"recursion_limit": 100, "max_concurrency": 4},
            },
        }
            ...
            "workflow": {
                ...
                "resource_requirements": {
                    ...
                    "network_required": any(
                        "api" in task.task_type.value for task in parsed_workflow.tasks
                    ),
                },
                "created_by": "Vatsal216",
        ...
        for task in parsed_workflow.tasks:
            if task.task_type.value == "decision":
                ...
                conditional_edge = {
                    ...
                    "metadata": {"type": "decision", "created_by": "Vatsal216"},
                }
    ...
    def _create_state_schema(self, parsed_workflow: ParsedWorkflow) -> Dict[str, Any]:
        ...
        return {"type": "object", "properties": state_properties, "required": ["workflow_state"]}
    ...
    def get_conversion_statistics(self) -> Dict[str, Any]:
        ...
        avg_input_length = (
            sum(c["input_length"] for c in self._conversion_history) / total_conversions
        )
...
class WorkflowToNLConverter:
    ...
    def convert_to_natural_language(
        self, workflow_def: Dict[str, Any], style: str = "descriptive"
    ) -> str:
        """
            ...
            for i, task in enumerate(tasks, 1):
                ...
                if style == "concise":
                    ...
                    description_parts.append(
                        f"{i}. {task_desc}, with the expected outcome being {expected_output.lower()}"
                    )
    ...
    def _convert_generic_to_nl(self, workflow_def: Dict[str, Any], style: str) -> str:
        ...
        if execution_plan:
            ...
            if parallel_groups:
                description_parts.append(
                    f"The workflow includes {len(parallel_groups)} groups of parallel tasks for improved efficiency."
                )
    ...
    def _record_conversion(
        self, workflow_def: Dict[str, Any], description: str, style: str, conversion_time: float
    ):
    ...
    def get_conversion_statistics(self) -> Dict[str, Any]:
        ...
        total_conversions = len(self._conversion_history)
        avg_conversion_time = (
            sum(c["conversion_time"] for c in self._conversion_history) / total_conversions
        )
        avg_description_length = (
            sum(c["description_length"] for c in self._conversion_history) / total_conversions
        )
