...
class SQLMemory(BaseMemory):
    ...
    def connect(self) -> None:
        ...
        try:
            ...
            self.engine = create_engine(
                self.database_url, echo=False, **self.pool_settings  # Set to True for SQL debugging
            )
            ...
            try:
                ...
                if existing_item:
                    ...
                    existing_item.metadata_json = json.dumps(
                        {"updated_at": datetime.utcnow().isoformat(), "updated_by": "Vatsal216"}
                    )
                else:
                    # Create new item
                    item = MemoryItem(
                        ...
                        metadata_json=json.dumps(
                            {"created_by": "Vatsal216", "created_at": datetime.utcnow().isoformat()}
                        ),
                    )
                    session.add(item)
        ...
        def _load():
            ...
            try:
                ...
                value = self._deserialize_value(
                    item.value_data, item.value_type, item.is_compressed
                )
    ...
    def _start_ttl_cleanup(self) -> None:
        ...
        self._cleanup_thread = threading.Thread(
            target=cleanup_worker, name="SQLMemory-TTL-Cleanup", daemon=True
        )
    ...
    def _cleanup_expired_items(self) -> int:
        ...
        try:
            ...
            deleted_count = (
                ...
                .filter(MemoryItem.expires_at.isnot(None), MemoryItem.expires_at <= current_time)
                .delete()
            )
    ...
    def get_database_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            expired_items = (
                session.query(MemoryItem)
                .filter(
                    MemoryItem.expires_at.isnot(None), MemoryItem.expires_at <= datetime.utcnow()
                )
                .count()
            )
    ...
    def restore_from_file(self, filepath: str, clear_existing: bool = False) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Restore completed: {restored_count} items restored, {failed_count} failed"
            )
