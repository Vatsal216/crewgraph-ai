...
class MemoryUtils:
    ...
    def benchmark_memory_backend(
        ...
        results["save"] = MemoryBenchmarkResult(
            ...
            average_latency_ms=sum(t for t in save_times if t != float("inf"))
            / len([t for t in save_times if t != float("inf")]),
        ...
        results["load"] = MemoryBenchmarkResult(
            ...
            average_latency_ms=sum(t for t in load_times if t != float("inf"))
            / len([t for t in load_times if t != float("inf")]),
        ...
        results["delete"] = MemoryBenchmarkResult(
            ...
            average_latency_ms=sum(t for t in delete_times if t != float("inf"))
            / len([t for t in delete_times if t != float("inf")]),
    ...
    def compare_memory_backends(
        configs: List[MemoryConfig], test_size: int = 1000
    ) -> Dict[str, Dict[str, MemoryBenchmarkResult]]:
        """
    ...
    def optimize_memory_config(workload_pattern: Dict[str, Any]) -> MemoryConfig:
        ...
        access_pattern = workload_pattern.get(
            "access_pattern", "random"
        )  # random, sequential, hotspot
        read_write_ratio = workload_pattern.get(
            "read_write_ratio", 0.8
        )  # 0.0 = all writes, 1.0 = all reads
