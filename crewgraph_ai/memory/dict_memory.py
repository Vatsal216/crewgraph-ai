...
class DictMemory(BaseMemory):
    ...
    def list_keys(self, pattern: Optional[str] = None) -> List[str]:
        ...
        return self._execute_with_metrics(
            MemoryOperation.LIST_KEYS,
            self._list_keys_impl,
            pattern
        )
    ...
    def _list_keys_impl(self, pattern: Optional[str] = None) -> List[str]:
        ...
        with self._lock:
            ...
            expired_keys = [
                key for key, exp_time in self._ttl_storage.items()
    ...
    def _save_to_file(self):
        ...
        try:
            with self._lock:
                ...
                current_time = time.time()
                valid_storage = {
                    key: value for key, value in self._storage.items()
                    if key not in self._ttl_storage or self._ttl_storage[key] > current_time
                }
                valid_ttl = {
                    key: exp_time for key, exp_time in self._ttl_storage.items()
    ...
    def get_memory_info(self) -> Dict[str, Any]:
        ...
        with self._lock:
            current_time = time.time()
            expired_count = sum(
                1 for exp_time in self._ttl_storage.values()
                if current_time > exp_time
            )
