    ...
    def search(
        ...
        def _search():
            ...
            if query.shape[1] != self.dimension:
                raise ValueError(
                    f"Query vector dimension {query.shape[1]} doesn't match index dimension {self.dimension}"
                )
            ...
            with self._lock:
                ...
                for i, (distance, idx) in enumerate(zip(distances[0], indices[0])):
                    ...
                    if filter_metadata:
                        ...
                        for key, value in filter_metadata.items():
                            if (
                                key not in vector_item.metadata
                                or vector_item.metadata[key] != value
                            ):
                                match = False
                                break
                        if not match:
                ...
                logger.debug(
                    f"FAISS search completed: {len(results)} results in {search_time:.3f}s"
                )
                return results
    ...
    def get_index_info(self) -> Dict[str, Any]:
        ...
        with self._lock:
            info = {
                ...
                "average_search_time": (
                    self._total_search_time / self._search_count if self._search_count > 0 else 0
                ),
