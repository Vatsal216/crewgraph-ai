...
class TaskWrapper:
    ...
    def execute(self, context: Optional[Dict[str, Any]] = None) -> TaskResult:
        ...
        if not self._check_dependencies():
            ...
            return TaskResult(task_id=self.id, task_name=self.name, success=False, error=error_msg)
    ...
    def execute_with_message_context(
    ...
    def execute_as_message_agent(
        self, messages: List[BaseMessage], state: Dict[str, Any] = None
    ) -> BaseMessage:
        """
    ...
    def create_message_from_result(self, result: TaskResult) -> AIMessage:
        ...
        if result.success:
            content = (
                str(result.result)
                if result.result
                else f"Task '{self.name}' completed successfully"
            )
        else:
    ...
    def update_context_with_messages(
        self, context: Dict[str, Any], messages: List[BaseMessage]
    ) -> Dict[str, Any]:
    ...
    def _execute_crew_task(self, context: Dict[str, Any]) -> Any:
        ...
        if hasattr(self.crew_task, "execute"):
            ...
            if self.assigned_agent and hasattr(self.assigned_agent, "crew_agent"):
                crew = Crew(
                    agents=[self.assigned_agent.crew_agent], tasks=[self.crew_task], verbose=True
                )
                result = crew.kickoff()
                return result
            else:
    ...
    def _execute_with_agent(self, context: Dict[str, Any]) -> Any:
        ...
        if hasattr(self.assigned_agent, "execute_task"):
            ...
            result = self.assigned_agent.execute_task(
                task_name=self.name, prompt=prompt, context=context, tools=self.tools
            )
