...
class AgentWrapper:
    ...
    def execute_task(self, 
        ...
        try:
            ...
            logger.info(f"Agent '{self.name}' completed task '{task_name}' in {execution_time:.2f}s")
    ...
    def _execute_with_crew_agent(self, 
                                prompt: str, 
        ...
        with self._lock:
            for agent in self._agents.values():
                ...
                if (agent.status == AgentStatus.IDLE and 
                    (not role or agent.role == role) and
                    agent.name not in self._active_agents):
                    return agent
