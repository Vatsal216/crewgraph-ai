...
class WorkflowOptimizer:
    ...
    def suggest_optimizations(
        ...
        resource_intensive_tasks = [t for t in tasks if t.get("estimated_duration", 0) > 20]
    ...
    def apply_optimization(
        self, workflow_definition: Dict[str, Any], optimization_result: OptimizationResult
    ) -> Dict[str, Any]:
        """
        ...
        if optimization_result.optimization_type == OptimizationType.PARALLELIZATION:
            optimized_workflow = self._apply_parallelization(
                optimized_workflow, optimization_result
            )

        elif optimization_result.optimization_type == OptimizationType.RESOURCE_ALLOCATION:
            optimized_workflow = self._apply_resource_optimization(
                optimized_workflow, optimization_result
            )

        elif optimization_result.optimization_type == OptimizationType.STRUCTURE:
            optimized_workflow = self._apply_structure_optimization(
                optimized_workflow, optimization_result
            )

        elif optimization_result.optimization_type == OptimizationType.CACHING:
            optimized_workflow = self._apply_caching_optimization(
                optimized_workflow, optimization_result
            )
    ...
    def get_optimization_impact(
        self, original_workflow: Dict[str, Any], optimized_workflow: Dict[str, Any]
    ) -> Dict[str, float]:
        """
    ...
    def _optimize_for_performance(
        self, workflow_definition: Dict[str, Any], constraints: Optional[Dict[str, Any]]
    ...
    def _optimize_for_resources(
        ...
        resource_intensive = [
            t
            for t in tasks
            if t.get("estimated_duration", 0) > 15 or t.get("type") in ["data_processing", "ml"]
        ]
    ...
    def _identify_parallelizable_tasks(
    ...
    def _apply_parallelization(
        self, workflow: Dict[str, Any], optimization: OptimizationResult
    ) -> Dict[str, Any]:
