...
class PerformanceDashboard:
    ...
    def __init__(self, config: Optional[DashboardConfig] = None):
        ...
        logger.info(
            f"PerformanceDashboard initialized with {len(self.config.metrics_to_track)} metrics"
        )
    ...
    def record_metrics(
        self, workflow_id: Optional[WorkflowId] = None, metrics: Optional[Dict[str, float]] = None
    ) -> None:
        """
    ...
    def generate_performance_report(self, time_range: Optional[timedelta] = None) -> Dict[str, Any]:
        """
        ...
        relevant_snapshots = [
            snapshot for snapshot in self.metrics_history if snapshot.timestamp >= cutoff_time
        ]
    ...
    def _analyze_workflow_patterns(self, snapshots: List[MetricSnapshot]) -> Dict[str, Any]:
        ...
        if workflow_groups:
            ...
            for workflow_id, workflow_snapshots in workflow_groups.items():
                avg_success = sum(s.success_rate for s in workflow_snapshots) / len(
                    workflow_snapshots
                )
            ...
            if workflow_performance:
                best_workflow = max(
                    workflow_performance.keys(), key=lambda w: workflow_performance[w]
                )
