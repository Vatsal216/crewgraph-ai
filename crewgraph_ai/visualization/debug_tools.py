...
class DebugTools:
    ...
    def analyze_dependencies(self, tasks: Dict[str, TaskWrapper]) -> Dict[str, Any]:
        ...
        if circular_deps:
            analysis["issues"].append(
                {"type": "circular_dependencies", "cycles": circular_deps, "severity": "error"}
            )
    ...
    def inspect_configuration(
        ...
        task_configs: Optional[Dict[str, Any]] = None,
    ) -> Dict[str, Any]:
        """
        ...
        config_issues = self._validate_configuration_values(
            workflow_config, agent_configs, task_configs
        )
    ...
    def identify_performance_bottlenecks(
        self, execution_data: Dict[str, Any], threshold_percentile: float = 0.95
    ...
    def generate_debug_report(
        self, workflow_data: Dict[str, Any], include_visualizations: bool = True
    ) -> str:
        """
        ...
        report = {
            ...
            "validation_summary": {
                ...
                "warning_count": len(
                    [i for i in self.validation_issues if i.severity == "warning"]
                ),
    ...
    def trace_execution_path(self, execution_trace: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
    ...
    def _validate_tasks(
        ...
        for task_id, task in tasks.items():
            try:
                ...
                self._add_validation_issue(
                    "error", "configuration", f"task_{task_id}", f"Error validating task: {str(e)}"
                )
        ...
        try:
            ...
            for cycle in cycles:
                self._add_validation_issue(
                    ...
                    details={"cycle": cycle},
                    suggestions=[
                        "Break circular dependency by removing or restructuring dependencies"
                    ],
                )
        except Exception as e:
            self._add_validation_issue(
                "warning", "dependency", "workflow", f"Could not analyze dependencies: {str(e)}"
            )
        ...
        try:
            ...
            if hasattr(state, "_access_log"):
                if len(state._access_log) == 0:
                    self._add_validation_issue(
                        "info", "usage", "state", "Shared state has not been accessed yet"
                    )

        except Exception as e:
            self._add_validation_issue(
                "error", "configuration", "state", f"Error validating state: {str(e)}"
            )
    ...
    def _validate_tools(self, tool_registry: ToolRegistry) -> None:
        ...
        try:
            ...
            self._add_validation_issue(
                "error", "configuration", "tools", f"Error validating tools: {str(e)}"
            )
            ...
            for node in nx.topological_sort(self.dependency_graph):
                if self.dependency_graph.in_degree(node) == 0:
                    ...
                    max_parent_depth = max(
                        depths[parent] for parent in self.dependency_graph.predecessors(node)
                    )
    ...
    def _analyze_execution_times(
        self, execution_data: Dict[str, Any], threshold_percentile: float
    ) -> None:
    ...
    def _validate_configuration_values(
        ...
        task_configs: Optional[Dict[str, Any]],
    ) -> List[Dict[str, Any]]:
        """Validate configuration values."""
        issues = []
    ...
    def _generate_dependency_recommendations(self, analysis: Dict[str, Any]) -> List[str]:
        ...
        if isolated_count > 0:
            recommendations.append(
                f"Review {isolated_count} isolated tasks for workflow integration"
            )
    ...
    def _generate_overall_recommendations(self) -> List[str]:
        ...
        if self.performance_bottlenecks:
            ...
            if high_severity_count > 0:
                recommendations.append(
                    f"Optimize {high_severity_count} high-severity performance bottlenecks"
                )
