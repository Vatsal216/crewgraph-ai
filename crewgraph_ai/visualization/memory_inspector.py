...
class MemoryInspector:
    ...
    def dump_memory_state(self, 
                         include_backend_details: bool = True,
                         include_gc_objects: bool = False) -> Dict[str, Any]:
        """
    ...
    def visualize_memory_usage(self, 
                              ...
                              title: str = "Memory Usage Analysis") -> str:
        """
        ...
        fig = make_subplots(
            rows=3, cols=2,
            subplot_titles=[
                "System Memory Usage", "Python Process Memory",
                "Object Count", "GC Collections",
                "Cache Hit Rate", "Memory Growth Rate"
            ],
    ...
    def detect_memory_leaks(self, threshold_mb: float = 100.0) -> Dict[str, Any]:
        ...
        timestamps = [(s.timestamp - recent_snapshots[0].timestamp).total_seconds() 
        ...
        analysis = {
            ...
            "potential_leak_detected": abs(slope_mb_per_hour) > threshold_mb / 24,  # Per day threshold
            "analysis_period_hours": (recent_snapshots[-1].timestamp - recent_snapshots[0].timestamp).total_seconds() / 3600,
            "recommendations": []
        }
        
        # Generate recommendations
        if analysis["potential_leak_detected"]:
            analysis["recommendations"].append("Memory growth detected - investigate object lifecycle")
            
        if object_growth > 10000:
            analysis["recommendations"].append("High object count growth - check for unreferenced objects")
            
        if gc_efficiency["efficiency_score"] < 0.5:
            analysis["recommendations"].append("Low GC efficiency - consider manual garbage collection")
    ...
    def export_analysis_report(self, format: str = "json") -> str:
        ...
        report = {
            ...
            "configuration": {
                ...
                "memory_backend_type": type(self.memory_backend).__name__ if self.memory_backend else None
            }
        }
    ...
    def _get_faiss_stats(self) -> Dict[str, Any]:
        ...
        try:
            if hasattr(self.memory_backend, 'index'):
                return {
                    'index_size': self.memory_backend.index.ntotal if hasattr(self.memory_backend.index, 'ntotal') else 0,
                    'is_trained': self.memory_backend.index.is_trained if hasattr(self.memory_backend.index, 'is_trained') else False
                }
        except Exception:
            pass
        return {}
    ...
    def _get_dict_stats(self) -> Dict[str, Any]:
        ...
        try:
            if hasattr(self.memory_backend, '_data'):
                ...
                return {
                    'key_count': len(data),
                    'estimated_size_bytes': sys.getsizeof(data)
                }
        except Exception:
            pass
        return {}
    ...
    def _analyze_gc_objects(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "type_distribution": dict(sorted(type_counts.items(), key=lambda x: x[1], reverse=True)[:20])
        }
    ...
    def _analyze_gc_efficiency(self) -> Dict[str, Any]:
        ...
        if len(set(object_counts)) > 1:  # Avoid division by zero
            efficiency_score = 1.0 - (max(memory_usage) - min(memory_usage)) / (max(object_counts) - min(object_counts)) * 100
