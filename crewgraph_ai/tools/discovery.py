...
class ToolDefinition:
    ...
    def to_crewai_tool(self) -> CrewAIBaseTool:
        ...
        return CrewAIBaseTool(name=self.name, func=self.function, description=self.description)
...
class ToolDiscovery:
    ...
    def __init__(self, config: Optional[DiscoveryConfig] = None):
        ...
        metrics.increment_counter(
            "crewgraph_tool_discovery_initialized_total", labels={"user": "Vatsal216"}
        )
    ...
    def discover_from_module(self, module_name: str) -> List[ToolDefinition]:
        ...
        try:
            ...
            metrics.record_duration(
                ...
                discovery_time,
                labels={"source_type": "module", "source": module_name, "user": "Vatsal216"},
            )
    ...
    def discover_from_directory(self, directory_path: str) -> List[ToolDefinition]:
        ...
        try:
            ...
            metrics.record_duration(
                "crewgraph_tool_discovery_duration_seconds",
                discovery_time,
                labels={"source_type": "directory", "source": directory_path, "user": "Vatsal216"},
            )
    ...
    def discover_from_package(self, package_name: str) -> List[ToolDefinition]:
        ...
        try:
            ...
            metrics.record_duration(
                "crewgraph_tool_discovery_duration_seconds",
                discovery_time,
                labels={"source_type": "package", "source": package_name, "user": "Vatsal216"},
            )
    ...
    def discover_all(self, auto_register: Optional[bool] = None) -> List[ToolDefinition]:
        ...
        metrics.record_gauge(
            "crewgraph_tools_discovered_count", len(validated_tools), labels={"user": "Vatsal216"}
        )

        logger.info(
            f"Discovery complete: {len(validated_tools)} tools discovered in {discovery_time:.2f}s"
        )

        return validated_tools

    def _analyze_function(
        self, func: Callable, module_name: str, func_name: str
        ...
        try:
            ...
            for method_name, method in inspect.getmembers(cls, predicate=inspect.isfunction):
                ...
                if hasattr(method, "_is_tool") or self._is_tool_method(method):
                    ...
                    tool_def = self._analyze_function(
                        tool_func, module_name, f"{class_name}.{method_name}"
                    )
                    if tool_def:
    ...
    def _extract_function_info_from_ast(
        ...
        try:
            ...
            for arg in node.args.args:
                parameters[arg.arg] = {"type": "Any", "required": True}
                ...
                for node in ast.walk(tree):
                    if isinstance(node, ast.FunctionDef):
                        ...
                        has_tool_decorator = any(
                            (isinstance(d, ast.Name) and d.id in ["tool", "crewai_tool"])
                            or (isinstance(d, ast.Attribute) and d.attr in ["tool", "crewai_tool"])
    ...
    def _filter_files(self, files: List[Path]) -> List[Path]:
        ...
        for file in files:
            ...
            excluded = any(pattern in file_str.lower() for pattern in self.config.exclude_patterns)
