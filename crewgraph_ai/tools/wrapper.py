...
class ToolWrapper(BaseTool):
        ...
        for attempt in range(self.max_retries + 1):
            try:
                ...
                if attempt < self.max_retries:
                    logger.warning(f"Tool {self.metadata.name} failed on attempt {attempt + 1}, retrying...")
                    metrics.increment_counter(
                    ...
                    logger.error(f"Tool {self.metadata.name} failed after {self.max_retries + 1} attempts")
                    metrics.increment_counter(
    ...
    def __repr__(self) -> str:
        return (f"ToolWrapper(name='{self.metadata.name}', "
