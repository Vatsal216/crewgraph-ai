...
class ToolRegistry:
    ...
    def register_tool(self, tool: BaseTool, force: bool = False) -> bool:
        ...
        with self._lock:
            ...
            if tool_name in self._tools and not force:
                logger.warning(
                    f"Tool '{tool_name}' already registered. Use force=True to override."
                )
                return False
    ...
    def list_tools(
        ...
        with self._lock:
            ...
            if tool_type:
                tools = [
                    name for name in tools if self._tools[name].metadata.tool_type == tool_type
                ]

            return sorted(tools)

    def search_tools(
        self, query: str, search_description: bool = True, search_tags: bool = True
    ) -> List[str]:
        """
    ...
    def get_recommendations(self, context: Dict[str, Any], limit: int = 5) -> List[Dict[str, Any]]:
        """
                    ...
                    recommendations.append(
                        {
                            ...
                            "success_rate": self._performance_metrics[name].get(
                                "success_rate", 1.0
                            ),
                        }
                    )
    ...
    def get_analytics_report(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            total_calls = sum(
                analytics.get("total_calls", 0) for analytics in self._usage_analytics.values()
            )
            ...
            category_dist = {category: len(tools) for category, tools in self._categories.items()}
            ...
            return {
                ...
                "most_used_tools": [
                    {"name": name, "calls": data.get("total_calls", 0)} for name, data in most_used
                ],
    ...
    def register_function(
        ...
        **metadata_kwargs,
        ...
        tool = ToolWrapper.from_function(
            func=func, name=name, description=description, **metadata_kwargs
        )

        return self.register_tool(tool)

    def register_crew_tool(
        self, crew_tool: Any, name: Optional[str] = None, **metadata_kwargs
    ) -> bool:
        """
        ...
        Returns:
            True if registered successfully
        """
        tool = ToolWrapper.from_crew_tool(crew_tool=crew_tool, name=name, **metadata_kwargs)
    ...
    def _calculate_recommendation_score(self, tool: BaseTool, context: Dict[str, Any]) -> float:
        ...
        max_usage = max(
            (a.get("total_calls", 0) for a in self._usage_analytics.values()), default=1
        )
        ...
        keyword_matches = sum(
            1 for keyword in context_keywords if keyword.lower() in description_words
        )
