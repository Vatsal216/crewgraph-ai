...
class ValidationConfig:
    ...
    allowed_imports: Set[str] = field(default_factory=lambda: {
        ...
        'requests', 'urllib', 're', 'typing', 'dataclasses', 'enum',
...
class ToolValidator:
    ...
    def validate_tool(self, tool: Union[Tool, Callable], tool_name: Optional[str] = None) -> ValidationResult:
        ...
        logger.info(f"Validation completed for {name}: {'PASSED' if result.is_valid else 'FAILED'} "
            ...
            if isinstance(node, ast.Call):
                ...
                if isinstance(node.func, ast.Attribute):
                    if isinstance(node.func.value, ast.Name):
                        if (node.func.value.id in network_modules or 
                            node.func.attr in network_functions):
                            result.issues.append(ValidationIssue(
                                severity=ValidationSeverity.MEDIUM,
                                category="security",
    ...
    def get_validation_report(self, results: Union[ValidationResult, List[ValidationResult], 
                                                  Dict[str, ValidationResult]]) -> str:
        """
    ...
    def export_results(self, results: Dict[str, ValidationResult], format_type: str = "json") -> str:
    ...
    def __repr__(self) -> str:
        return (f"ToolValidator(level={self.config.validation_level.value}, "
