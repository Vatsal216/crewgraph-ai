...
class AuthenticationManager:
    ...
    def __init__(self, jwt_secret: str, session_timeout: int = 3600, max_failed_attempts: int = 5):
        """
    ...
    def create_token(
        self, user_id: str, permissions: List[str], expires_in: Optional[int] = None
    ) -> str:
        """
    ...
    def refresh_token(self, token: str) -> Optional[str]:
        ...
        if payload:
            ...
            new_token = self.create_token(payload["user_id"], payload["permissions"])
...
class SecurityManager:
    ...
    def authenticate_user(
        ...
        if not self.auth.check_rate_limit(username):
            self.audit_log(
                "authentication_failed", {"username": username, "reason": "rate_limited"}
            )
            return None
    ...
    def authorize_operation(
        ...
        if not payload:
            self.audit_log(
                ...
                {"operation": operation, "resource": resource, "reason": "invalid_token"},
            )
            return False
        ...
        has_permission = required_perm in user_permissions or "admin" in user_permissions
    ...
    def add_user_permission(self, user_id: str, permission: str) -> None:
        ...
        self.audit_log("permission_granted", {"user_id": user_id, "permission": permission})
    ...
    def remove_user_permission(self, user_id: str, permission: str) -> None:
        ...
        self.audit_log("permission_revoked", {"user_id": user_id, "permission": permission})

    def create_role(self, role_name: str, permissions: List[str]) -> None:
        """Create role with permissions"""
        self._role_permissions[role_name] = permissions

        self.audit_log("role_created", {"role_name": role_name, "permissions": permissions})
    ...
    def assign_role_to_user(self, user_id: str, role_name: str) -> None:
        ...
        if role_name in self._role_permissions:
            ...
            self.audit_log("role_assigned", {"user_id": user_id, "role_name": role_name})
...
def initialize_security(
    ...
    _global_security_manager = SecurityManager(
        policy=policy, encryption_key=encryption_key, jwt_secret=jwt_secret
    )
