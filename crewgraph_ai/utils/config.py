...
class CrewGraphConfig:
    ...
    def _load_from_environment(self):
        ...
        # Security
        self.security.encryption_key = os.getenv("CREWGRAPH_ENCRYPTION_KEY", self.security.encryption_key)
        ...
        # Workflow
        self.workflow.max_concurrent_workflows = int(os.getenv(
            "MAX_CONCURRENT_WORKFLOWS", 
            str(self.workflow.max_concurrent_workflows)
        ))
        self.workflow.max_concurrent_tasks = int(os.getenv(
            "MAX_CONCURRENT_TASKS",
            str(self.workflow.max_concurrent_tasks)
        ))
    ...
    def _validate_config(self):
        ...
        if self.memory.backend not in valid_backends:
            errors.append(f"Invalid memory backend: {self.memory.backend}. Must be one of {valid_backends}")
    ...
    def get_database_url(self) -> str:
        ...
        return (f"postgresql://{auth}@{self.database.host}:{self.database.port}/"
...
def load_config(config_path: Optional[Union[str, Path]] = None) -> CrewGraphConfig:
    ...
    if config_path and os.path.exists(config_path):
        ...
        logger.info("No configuration file found, using default configuration with environment variables")
...
def _dict_to_config(config_data: Dict[str, Any]) -> CrewGraphConfig:
    ...
    main_config = {
        ...
        if k not in ['database', 'redis', 'security', 'monitoring', 'api', 'workflow', 'logging', 'memory']
    }
