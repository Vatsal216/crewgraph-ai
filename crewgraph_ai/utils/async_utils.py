...
class AsyncTaskManager:
    ...
    def __init__(self, max_concurrent_tasks: int = 10, default_timeout: float = 300.0):
        ...
        logger.info(
            f"AsyncTaskManager initialized with max_concurrent_tasks={max_concurrent_tasks}"
        )
    ...
    async def start(self) -> None:
        ...
        for priority in TaskPriority:
            worker_task = asyncio.create_task(
                self._worker_loop(priority), name=f"worker_{priority.name.lower()}"
            )
    ...
    async def submit_task(
        ...
        metrics.increment_counter(
            "async_tasks_submitted_total", labels={"priority": priority.name.lower()}
        )
        ...
        while True:
            ...
            if task_id in self._failed_tasks:
                ...
                raise ExecutionError(
                    f"Task {task_id} failed: {error_info['error']}", details=error_info
                )
    ...
    async def _execute_task(self, task: AsyncTask) -> None:
        ...
        try:
            ...
            if task.retry_count < task.max_retries:
                task.retry_count += 1
                logger.warning(
                    f"Task {task_id} failed, retrying ({task.retry_count}/{task.max_retries}): {e}"
                )
...
class BatchProcessor:
    ...
    def __init__(
        ...
        logger.info(
            f"BatchProcessor initialized: batch_size={batch_size}, max_concurrent={max_concurrent_batches}"
        )
    ...
    async def process_batch(
        ...
        logger.info(
            f"Batch processing completed: {processing_summary['successful']}/{total_items} successful"
        )
        return processing_summary

    async def _process_single_batch(
        self, batch: List[Any], batch_idx: int, processor_func: Callable[[Any], Awaitable[Any]]
    ) -> Dict[str, Any]:
        ...
        async with self._semaphore:
            ...
            for i, item_task in enumerate(asyncio.as_completed(item_tasks)):
                try:
                    ...
                    successful.append(
                        {"item": batch[i], "result": result, "batch_idx": batch_idx, "item_idx": i}
                    )
                except Exception as e:
                    failed.append(
                        {"item": batch[i], "error": str(e), "batch_idx": batch_idx, "item_idx": i}
                    )

            return {"successful": successful, "failed": failed, "processed": len(batch)}

    async def _retry_failed_items(
        self, failed_items: List[Dict[str, Any]], processor_func: Callable[[Any], Awaitable[Any]]
    ) -> Dict[str, Any]:
        ...
        for failed_item in failed_items:
            try:
                ...
                successful.append({"item": failed_item["item"], "result": result, "retry": True})
            except Exception as e:
                still_failed.append(
                    {"item": failed_item["item"], "error": str(e), "retry_failed": True}
                )

        return {"successful": successful, "failed": still_failed}
...
async def gather_with_concurrency_limit(coroutines: List[Awaitable[T]], limit: int = 10) -> List[T]:
